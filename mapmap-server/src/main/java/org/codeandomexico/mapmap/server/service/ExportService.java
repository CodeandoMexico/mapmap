...
public class ExportService {
	...
	public Optional<File> generateExportCsv(Set<String> unitIds, String timestamp) {
		...
		try {
			...
			String[] routesHeader = "unit_id, route_id, route_name, route_description, field_notes, vehicle_type, vehicle_capacity, start_capture"
					.split(",");
			...
			String[] stopsHeader = "route_id, stop_sequence, lat, lon, travel_time, dwell_time, board, allright"
					.split(",");
	...
	private void processStopsShapefile(File outputDirectory, String exportName, List<TripPattern> tripPatterns)
		...
		SimpleFeatureType STOP_TYPE = DataUtilities.createType("Stop",
				"the_geom:Point:srid=4326," + "STOP_ID:String," + "ROUTE_ID:String," + "SEQUENCE:Integer,"
						+ "TRAVEL_S:Integer," + "DWELL_S:Integer," + "ARRIVAL_S:Integer," + "ARRIVAL_T:String,"
						+ "DEPARTUR_S:Integer," + "DEPARTUR_T:String," + "BOARD:Integer," + "ALIGHT:Integer,"
						+ "PASSENGERS:Integer");
	...
	private void processRoutesShepfile(File outputDirectory, String exportName, List<TripPattern> tripPatterns)
			throws IOException, SchemaException {
		...
		SimpleFeatureType ROUTE_TYPE = DataUtilities.createType("Route",
				"the_geom:LineString:srid=4326," + "ROUTE_ID:String," + "UNIT_ID:String," + "UNIT_OWNER:String,"
						+ "NAME:String," + "DESC:String," + "NOTES:String," + "START:String");
		dataStore.createSchema(ROUTE_TYPE);

		List<SimpleFeature> simpleFeatures = tripPatterns.stream().filter(tripPattern -> tripPattern.tripShape != null)
				.map(tripPattern -> {
					...
					simpleFeatureBuilder.add(
							tripPattern.route.phone.userName != null ? tripPattern.route.phone.userName.toUpperCase()
									: "");
					simpleFeatureBuilder
							.add(tripPattern.route.routeLongName != null ? tripPattern.route.routeLongName.toUpperCase()
									: "");
					simpleFeatureBuilder
							.add(tripPattern.route.routeDesc != null ? tripPattern.route.routeDesc.toUpperCase() : "");
					simpleFeatureBuilder.add(
							tripPattern.route.routeNotes != null ? tripPattern.route.routeNotes.toUpperCase() : "");
					simpleFeatureBuilder
							.add(tripPattern.route.captureTime != null ? tripPattern.route.captureTime.toString() : "");
